...
def try_on():
    ...
    if not data or 'person_filename' not in data or 'garment_filename' not in data:
        return jsonify({'error': '缺少必要的文件名参数'}), 400
        
    person_filepath = os.path.join(UPLOAD_FOLDER, data['person_filename'])
    garment_filepath = os.path.join(UPLOAD_FOLDER, data['garment_filename'])
    
    if not os.path.exists(person_filepath) or not os.path.exists(garment_filepath):
        return jsonify({'error': '文件不存在'}), 404
        
    try:
        # 1. 读取图像
        person_img = cv2.imread(person_filepath)
        garment_img = cv2.imread(garment_filepath)
        
        result_id = generate_result_id()
        
        # 2. 3D 人体重建
        print("步骤 1: 3D 人体重建...")
        pose_2d = pose_2d_estimator.estimate(person_img)
        pose_3d = pose_3d_reconstructor.reconstruct(person_img, keypoints_2d=pose_2d.landmarks_pixel if pose_2d else None)
        
        # 3. 服装分析与重建
        print("步骤 2: 服装分析与 3D 化...")
        garment_region = garment_segmenter.segment_standalone_garment(garment_img)
        if garment_region is None:
            return jsonify({'error': '未能检测到服装'}), 400
        ...
        # 4. 执行试穿适配 (LBS + Warp)
        print("步骤 3: 服装适配与 LBS 变形...")
        fitted_garment = garment_3d_reconstructor.drape_on_body(
            garment_3d,
            pose_3d.vertices,
            pose_3d.faces,
            pose_3d.joints_3d,
            smpl_params=pose_3d.lbs_data
        )
        
        # 5. 生成可视化结果
        print("步骤 4: 生成渲染结果...")
        try_on_vis = visualizer.visualize_try_on(
            pose_3d.vertices,
            pose_3d.faces,
            fitted_garment.vertices,
            fitted_garment.faces,
            title="3D Virtual Try-On Result"
        )
        
        # 导出模型
        mesh_path = os.path.join(RESULTS_FOLDER, f"{result_id}_tryon.obj")
        garment_3d_reconstructor.export_obj(fitted_garment, mesh_path)
        
        return jsonify({
            'success': True,
            'result_id': result_id,
            'visualization': try_on_vis,
            'mesh_file': f"{result_id}_tryon.obj",
            'message': '虚拟试穿完成'
        })
        
    except Exception as e:
        traceback.print_exc()
        return jsonify({'error': f'试穿失败: {str(e)}'}), 500
