...
class Pose3DResult:
    ...
    confidence: float
    # LBS所需额外数据
    lbs_data: Dict = field(default_factory=dict)
...
class SimpleSMPL(nn.Module):
    ...
    def forward(
        ...
        Returns:
            ...
            joints: 关节坐标 (B, 24, 3)
            extra_data: 包含 LBS 矩阵等
        """
        batch_size = pose.size(0)
        device = pose.device
        
        # 1. 应用形状变形 (Shape Blend Shapes)
        v_template = self.v_template.to(device)
        shapedirs = self.shapedirs.to(device)
        
        if len(shapedirs.shape) == 3:
            v_shaped = v_template.unsqueeze(0) + torch.einsum('vij,bj->bvi', shapedirs, shape)
        else:
            v_shaped = v_template.unsqueeze(0) + torch.matmul(shape, shapedirs.T).view(batch_size, -1, 3)
            
        # 2. 计算关节位置 (Joints from shaped vertices)
        J_regressor = self.J_regressor.to(device)
        joints = torch.matmul(J_regressor, v_shaped)
        
        # 3. 应用姿态变形 (Pose Blend Shapes)
        # 转换轴角为旋转矩阵
        ident = torch.eye(3, device=device).unsqueeze(0).unsqueeze(0)
        pose_matrix = self._axis_angle_to_matrix(pose.view(-1, 3)).view(batch_size, -1, 3, 3)
        
        # 计算旋转矩阵相对于身份矩阵的偏移
        pose_feature = (pose_matrix[:, 1:, :, :] - ident).view(batch_size, -1)
        posedirs = self.posedirs.to(device)
        
        if posedirs.shape[0] == self.NUM_VERTICES:
            # 完整版姿态变形 (V, 3, 207)
            v_posed = v_shaped + torch.einsum('vpk,bp->bvk', posedirs, pose_feature)
        else:
            v_posed = v_shaped
            
        # 4. 正向运动学 (Forward Kinematics)
        # 获取每个关节相对于父关节的旋转
        # parents 定义了骨骼树结构
        parents = self.parents.to(device)
        
        # 存储全局变换矩阵 (B, 24, 4, 4)
        global_transforms = torch.zeros(batch_size, self.NUM_JOINTS, 4, 4, device=device)
        
        # 根关节处理
        root_rot = pose_matrix[:, 0, :, :]
        root_j = joints[:, 0, :].unsqueeze(2)
        global_transforms[:, 0, :3, :3] = root_rot
        global_transforms[:, 0, :3, 3:4] = root_j
        global_transforms[:, 0, 3, 3] = 1.0
        ...
        for i in range(1, self.NUM_JOINTS):
            ...
            rot = pose_matrix[:, i, :, :]
            # 关节在父空间中的偏移
            j_offset = (joints[:, i, :] - joints[:, parent, :]).unsqueeze(2)
            
            local_transform = torch.zeros(batch_size, 4, 4, device=device)
            local_transform[:, :3, :3] = rot
            local_transform[:, :3, 3:4] = j_offset
            local_transform[:, 3, 3] = 1.0
            
            # 全局变换 = 父全局变换 * 局部变换
            global_transforms[:, i] = torch.matmul(global_transforms[:, parent], local_transform)
            
        # 5. 线性混合蒙皮 (Linear Blend Skinning, LBS)
        # 移除关节在休息姿态下的位置偏移
        # T_prime = T_global * T_rest_inv
        rest_joints = torch.zeros(batch_size, self.NUM_JOINTS, 4, 4, device=device)
        rest_joints[:, :, :3, 3:4] = -joints.unsqueeze(3)
        rest_joints[:, :, :3, :3] = torch.eye(3, device=device)
        rest_joints[:, :, 3, 3] = 1.0
        
        # 蒙皮矩阵 (B, 24, 4, 4)
        W = torch.matmul(global_transforms, rest_joints)
        
        # 应用混合权重
        weights = self.weights.to(device) # (V, 24)
        # 为每个顶点计算最终变换 (B, V, 4, 4)
        T = torch.einsum('vj,bjkl->bvkl', weights, W)
        
        # 转换顶点坐标为齐次坐标
        v_posed_homo = torch.cat([v_posed, torch.ones(batch_size, self.NUM_VERTICES, 1, device=device)], dim=2)
        
        # 应用最终变换
        v_final_homo = torch.matmul(T, v_posed_homo.unsqueeze(3))
        vertices = v_final_homo[:, :, :3, 0]
        
        # 更新关节位置到最终姿态
        final_joints = global_transforms[:, :, :3, 3]
        
        # 6. 应用全局平移
        if translation is not None:
            vertices = vertices + translation.unsqueeze(1)
            final_joints = final_joints + translation.unsqueeze(1)
        
        extra_data = {
            'pose_matrices': W,
            'rest_vertices': v_shaped,
            'skinning_weights': weights
        }
            
        return vertices, final_joints, extra_data
...
class Pose3DReconstructor:
    ...
    def reconstruct(self, image: np.ndarray, keypoints_2d: Optional[np.ndarray] = None) -> Pose3DResult:
        ...
        # 生成3D网格和关节
        vertices, joints, extra_data = self.smpl(pose, shape)
        ...
        # 转换 LBS 数据
        lbs_data = {
            'pose_matrices': extra_data['pose_matrices'].cpu().numpy()[0],
            'rest_vertices': extra_data['rest_vertices'].cpu().numpy()[0],
            'weights': extra_data['skinning_weights'].cpu().numpy()
        }
